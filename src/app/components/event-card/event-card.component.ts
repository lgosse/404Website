...
export class EventCardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.mailingListsService
      .getMailingListByName(this.event.title)
      .subscribe(list => {
  ...
  getUserInfos() {
    this.userService.user.subscribe(user => {
      ...
      this.subscriptionService
        .getSubscriptionsEventLogins(this.event.title)
        .subscribe(logins => {
          this.logins = logins;
  ...
  unsubscribe(): void {
    ...
    this.subscriptionService.removeSubscriptionsEventLogin(
      this.loginKey,
      this.eventKey
    );
